...
BEGIN
    $3
    return 0;
END;`}}class cn extends Ue{showVersion(){return"select replace(@@version,'-MariaDB','') server_version;"}}class mn extends Pe{showVersion(){return"show version"}showDatabases(){return"show dbs"}buildPageSql(e,t,n){return`db('${e}').collection('${t}').find({}).limit(${n}).toArray()`}showIndex(e,t){return`db('${e}').collection('${t}').indexes();`}createIndex(e){const{database:t,table:n,column:r}=e;return`db('${t}').collection('${n}').createIndex({ ${r||"column_name"}: 1 });`}createDatabase(e){return'db("db_name").createCollection("collection")'}}class Tn extends Ee{showVersion(){return"SELECT CAST(SERVERPROPERTY('ProductVersion') AS SYSNAME)+' '+CAST(SERVERPROPERTY('Edition') AS SYSNAME) AS server_version"}showAllForeignKeys(){return`SELECT 
...
BEGIN
    ...
    return 0;
END;
$$ LANGUAGE plpgsql;`}}class dn extends xe{showVersion(){return"SELECT VERSION() as server_version"}createIndex(e){throw new Error("Redshift not support index!")}variableList(){return"SHOW ALL"}statusList(){return`SELECT
        'db_numbackends' AS db,
        pg_stat_get_db_numbackends(datid) AS status
      FROM
        pg_stat_database
...
-- since mysql version 5.7, password column need change to authentication_string=PASSWORD("newPassword")`}updateColumn(e,t){var N;const{name:n,type:r,comment:a,nullable:l,defaultValue:o,extra:i,character_set_name:m,collation_name:O}=t,S=l!="YES";return new k(`ALTER TABLE ${e}`).append(`
	CHANGE ${n} ${n} ${r}`).if(m,`CHARACTER SET ${m}`).if(O,`COLLATE ${O}`).if(S,"NOT NULL").if((N=i==null?void 0:i.toLowerCase())==null?void 0:N.includes("auto_increment"),"AUTO_INCREMENT").if(a,`COMMENT '${a}'`).if(K(o)&&!S,"DEFAULT NULL").if(!K(o),`DEFAULT ${o=="CURRENT_TIMESTAMP"?o:`'${mt(o)}'`}`).toString()}updateColumnSql(e){const{table:t,columnName:n,newColumnName:r,columnType:a,isNotNull:l,isAutoIncrement:o,comment:i,defaultValue:m,oldRow:O}=e,S=`ALTER TABLE "${t}"`,N=l?"SET NOT NULL":"DROP NOT NULL";return new k(`${S} ALTER COLUMN "${n}" TYPE ${a};`,`
...
from information_schema.referential_constraints rco
    ...
    ORDER BY rco.CREATED`}}class Un extends _e{newIndex(e){const{table:t=this.meta.table,columns:n}=e;return`CREATE INDEX ${`${t}_${n.join("_")}_${new Date().getTime()}`} ON "${t}"(${this.splitColumn(n)});`}showForeignKeys(){return`SELECT "from" column_name, "table" referenced_table, "to" referenced_column,
        on_update, on_delete 
        FROM pragma_foreign_key_list('${this.meta.table}');`}showChecks(){return null}}class yn extends _e{newColumn(e){const{type:t,defaultValue:n}=e;return new k(super.newColumn(e)).if(!K(n),`DEFAULT ${ae(n,t)}`).toString()}newIndex(e){const{table:t=this.meta.table,columns:n}=e;return`CREATE INDEX ${`${t}_${n.join("_")}_${new Date().getTime()}`} ON "${t}"(${this.splitColumn(n)});`}showForeignKeys(){return`SELECT constraint_column_names "column_name" FROM duckdb_constraints()
        WHERE constraint_type='FOREIGN KEY' and database_name not in ('system','temp') and schema_name='${this.meta.schema}' and table_name='${this.meta.table}'`}showChecks(){return null}}class Bn extends Ve{columnTypes(){return["INTEGER",{label:"VARCHAR",value:"VARCHAR(255)"},{label:"CHAR",value:"CHAR(50)"},"hr","DATE","TIMESTAMP","TIME","hr","BOOLEAN","FLOAT","DOUBLE","hr","TEXT","BLOB","hr","SMALLINT","BIGINT","DECFLOAT","hr","TIMESTAMP WITH TIME ZONE","TIME WITH TIME ZONE","hr","XML","GRAPHIC","VARGRAPHIC","hr","DBCLOB","CLOB"]}}class vn extends Ve{columnTypes(){return["INTEGER","TEXT","hr","DATE","TIMESTAMP","hr","BOOLEAN","FLOAT","DOUBLE","hr","BLOB","hr","BIGINT","SMALLINT","TINYINT","VARINT","hr","TIME","TIMEUUID","hr","DECIMAL","COUNTER","hr","UUID","INET","ASCII","DURATION"]}dropColumn(e){return`ALTER TABLE ${this.wrappedTable} DROP ${ue(e,this.meta.dbType)}`}}function Fn(u){switch(u.dbType){case R.PG:case R.REDSHIFT:return new wn(u);case R.DM:case R.ORACLE:return new Dn(u);case R.DB2:return new Bn(u);case R.CASSANDRA:return new vn(u);case R.SQLServer:return new Ve(u);case R.SNOWFLAKE:return new gn(u);case R.CLICKHOUSE:return new hn(u);case R.DUCK_DB:return new yn(u);case R.SQLITE:return new Un(u);case R.MYSQL:case R.MARIA_DB:default:return new fn(u)}}const Hn={class:"mt-2 design-container"},Pn={id:"infoPanel"},xn={class:"tab"},Vn=["onClick"],kn={class:"design-data-container"},Yn=ce({__name:"design",setup(u){const e=Te(),t=P(()=>e.activePanel),n=P(()=>[{id:"ddl",i18n:"design.ddl",icon:"codicon-edit green"},{id:"column",i18n:"design.column",icon:"codicon-symbol-field blue"},{id:"foreignKey",i18n:"design.foreignKey",icon:"codicon-symbol-class yellow",hidden:P(()=>!e.supportForeignKey)},{id:"index",i18n:"design.index",icon:"codicon-github-action purple",hidden:P(()=>!e.supportIndex)},{id:"trigger",i18n:"design.trigger",icon:"codicon-github-action orange",hidden:P(()=>!e.supportTrigger)},{id:"check",i18n:"design.check",icon:"codicon-tools red",hidden:P(()=>!e.supportCheck)}].filter(a=>{var l;return((l=a==null?void 0:a.hidden)==null?void 0:l.value)!=!0}));me(()=>{D.on("dbType",a=>{e.updateDBType(a)}).on("designMeta",a=>{e.dialect=Fn(a),e.updateFullInfo(a),D.emitImmediately("designMetaNext",a)}).on("success",()=>{qe(T("design.updateSuccess"))}).on("error",a=>{De(a)}).on("refresh_design",()=>{r()}),r(),dt(55,["tableTab","infoPanel"],a=>{e.remainHeight=a})});const r=()=>{D.emit("init",{type:"design"})};return(a,l)=>(_(),w("div",Hn,[I("div",Pn,[s(zt),I("ul",xn,[(_(!0),w(z,null,se(n.value,(o,i)=>(_(),w("li",{key:i,class:Ye(["tab__item",{"tab__item--active":t.value==o.id}]),onClick:m=>E(e).activePanel=o.id},[o.icon?(_(),w("i",{key:0,type:"icon",class:Ye(o.icon),style:{position:"relative",top:"3px"}},null,2)):h("",!0),U(" "+g(E(T)(o.i18n)),1)],10,Vn))),128))])]),I("div",kn,[Y(s(Ot,null,null,512),[[te,t.value=="ddl"]]),Y(s(Ht,null,null,512),[[te,t.value=="column"]]),Y(s(ht,null,null,512),[[te,t.value=="foreignKey"]]),Y(s(St,null,null,512),[[te,t.value=="index"]]),Y(s(on,null,null,512),[[te,t.value=="trigger"]]),Y(s(an,null,null,512),[[te,t.value=="check"]])])]))}}),pa=Ke(Yn,[["__scopeId","data-v-ef75f437"]]);export{pa as default};
